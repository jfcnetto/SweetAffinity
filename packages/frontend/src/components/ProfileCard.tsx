...
// Função auxiliar para calcular a idade em tempo real baseada no ano de 2026
const calculateAge = (birthDateString: string): number => {
  if (!birthDateString) return 18;
  const today = new Date('2026-06-28'); // Mantém o sincronismo temporal estável do sistema
...
const ProfileCard: React.FC<{ profile: Profile; isFavorited: boolean; onToggleFavorite: (e: React.MouseEvent) => void; }> = ({ profile, isFavorited, onToggleFavorite }) => {
  ...
  // Caminho da imagem apontando para o MinIO local ou fallback visual caso o perfil não tenha foto ainda
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg shadow-lg h-80">
      ...
      <div className="absolute bottom-0 left-0 p-4 text-white">
        <h3 className="text-xl font-bold">{profile.display_name}, {age}</h3>
        <p className="text-sm text-gray-300">{profile.city || 'Brasil'}</p>
      </div>
      <button
      ...
      {/* Mapeia a propriedade de ativação/verificação da conta conforme nossa estrutura local */}
      {profile.is_active && (
