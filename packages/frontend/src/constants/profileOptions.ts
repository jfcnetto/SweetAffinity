export const PROFILE_TYPES = [
    { value: 'baby', label: 'Sugar Baby' },
    { value: 'daddy', label: 'Sugar Daddy' },
    { value: 'mommy', label: 'Sugar Mommy' }
];

export const SEEKING_GENDER = [
    { value: 'HOMEM', label: 'Homens' },
    { value: 'MULHERES', label: 'Mulheres' },
    { value: 'HOMENS E MULHERES', label: 'Homens e Mulheres' }
];

export const MARITAL_STATUSES = [
    'Solteiro(a)', 
    'Separado(a)', 
    'Divorciado(a)', 
    'Viúvo(a)', 
    'Em um relacionamento'
];

export const CHILDREN_OPTIONS = [
    'Nenhum', 
    '1 filho', 
    '2 filhos', 
    '3 ou mais filhos'
];

export const HEIGHT_OPTIONS = [
    'Menos de 1.50m', 
    '1.50m - 1.60m', 
    '1.60m - 1.70m', 
    '1.70m - 1.80m', 
    '1.80m - 1.90m', 
    'Mais de 1.90m'
];

export const BODY_TYPES = [
    'Atlético', 
    'Magro', 
    'Em forma', 
    'Médio', 
    'Curvilíneo', 
    'Alguns quilos a mais', 
    'Gordinho', 
    'Acima do peso', 
    'Musculoso'
];

export const ETHNICITIES = [
    'Asiático', 
    'Branco/Caucasiano', 
    'Indígena', 
    'Latino/Hispânico', 
    'Negro/Afrodescendente', 
    'Outro'
];

export const HAIR_COLORS = [
    'Preto', 
    'Castanho', 
    'Loiro', 
    'Ruivo', 
    'Grisalho', 
    'Outro'
];

export const EYE_COLORS = [
    'Preto', 
    'Castanho', 
    'Azul', 
    'Verde', 
    'Cor de mel', 
    'Outro'
];

export const SMOKING_OPTIONS = [
    'Nunca', 
    'Socialmente', 
    'Frequentemente'
];

export const DRINKING_OPTIONS = [
    'Nunca', 
    'Socialmente', 
    'Frequentemente'
];

export const TRAVEL_OPTIONS = [
    'Localmente', 
    'Nacionalmente', 
    'Internacionalmente', 
    'Mundo inteiro'
];

export const EDUCATION_LEVELS = [
    'Ensino Médio', 
    'Superior Incompleto', 
    'Superior Completo', 
    'Pós-graduação', 
    'Mestrado', 
    'Doutorado'
];

export const PROFESSION_OPTIONS = [
    'Administração/Negócios',
    'Arte/Cultura/Design',
    'Autônomo(a)/Empresário(a)',
    'Ciência/Tecnologia',
    'Comunicação/Mídia',
    'Direito/Advocacia',
    'Educação/Ensino',
    'Engenharia/Arquitetura',
    'Estudante',
    'Finanças/Contabilidade',
    'Governo/Setor Público',
    'Marketing/Vendas',
    'Medicina/Saúde',
    'Moda/Beleza',
    'Técnico/Operacional',
    'Outro'
];

export const INCOME_RANGES = [
    'Até R$ 50 mil', 
    'R$ 50 mil a R$ 100 mil', 
    'R$ 100 mil a R$ 250 mil', 
    'R$ 250 mil a R$ 500 mil', 
    'Acima de R$ 500 mil'
];

export const NET_WORTH_RANGES = [
    'Até R$ 100 mil', 
    'R$ 100 mil a R$ 500 mil', 
    'R$ 500 mil a R$ 1 milhão', 
    'R$ 1 milhão a R$ 5 milhões', 
    'Acima de R$ 5 milhões'
];

export const PARTNERSHIP_TYPES = [
    { value: 'companionship', label: 'Companheirismo & Presença' },
    { value: 'financial', label: 'Suporte Financeiro / Subsídio' },
    { value: 'mentorship', label: 'Mentoria & Networking' },
    { value: 'travel', label: 'Viagens & Aventuras' },
    { value: 'other', label: 'Outro' }
];

export const MEETING_FREQUENCIES = [
    { value: 'flexible', label: 'Flexível / Casual' },
    { value: 'weekly', label: 'Semanal' },
    { value: 'bi_weekly', label: 'Quinzenal' },
    { value: 'monthly', label: 'Mensal' },
    { value: 'multi_weekly', label: 'Múltiplas vezes na semana' }
];
